...
def api_request(api_key, api_choice, currency_choice):
    ...
    if api_choice == '1':
        headings, data, filepath = api_request_cmc(api_key, currency_choice)
        return headings, data, filepath

    elif api_choice == '2':
        headings, data, filepath = api_request_cg(api_key, currency_choice)
        return headings, data, filepath

def api_request_cmc(api_key, currency_choice):
    parameters = {
        'start':'1',
        'limit':'14',
    ...
    try:
        ...
        for coin in cmc_coins:
            ...
            output.append({
                ...
                '#': ranking,
                'Name': name,
                'Current Price ('+currency_choice+')': f"{price:,.2f}",
                '1h%': f"{volume_1h:,.2f}",
                '24h%': f"{volume_24h:,.2f}",
                '7d%': f"{volume_7d:,.2f}",
